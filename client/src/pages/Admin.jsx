...
function Admin() {
    ...
    const [staffInfo, setStaffInfo] = useState({ username: '', password: '' });
    ...
    const handleUsernameChange = (e) => {
        setStaffInfo(prevState => ({ ...prevState, username: e.target.value }));
    }
  
    const handlePasswordChange = (e) => {
        setStaffInfo(prevState => ({ ...prevState, password: e.target.value }));
    }
      ...
      <div className='bg-gray-100 flex items-center justify-center h-screen'>
        ...
        <div className='bg-white shadow-lg shadow-slate-200 outline outline-1 outline-gray-300 rounded-sm p-8 w-full md:w-96'>
          ...
          <form className='mt-12 space-y-6' onSubmit={handleSubmit}>
            ...
            <input type='text' value={staffInfo.username} onChange={handleUsernameChange} placeholder='Username' className='w-full h-16 outline outline-1 outline-stone-600 rounded-sm p-3 text-lg' required/>
            <div className='relative'>
              <input type={showPassword ? 'text' : 'password'} value={staffInfo.password} onChange={handlePasswordChange} placeholder='Add a Password' className='w-full h-16 outline outline-1 outline-stone-600 rounded-sm p-3 text-lg' required/>
