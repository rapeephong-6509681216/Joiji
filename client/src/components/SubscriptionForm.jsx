...
function SubscriptionForm( {setSubscription, setStep} ) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        e.stopPropagation();
        setSubscription({subType: type[selected], price: price[selected] + ".99", quota: disc[selected]});
        setStep(3);
    }
  ...
  return (
    <div>
        ...
        <div className='flex justify-center items-center'>
            <button type="submit" onClick={handleSubmit} className='mt-5 bg-red-netflix text-white font-normal text-2xl rounded w-96 h-16 active:bg-red-netflix-active'>Next</button>
        </div>
    </div>
  )
}
